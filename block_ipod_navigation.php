...
class ipod_navigation extends global_navigation {
   ...
   /**
    ...
    public function initialise() {
        ...
        $coursenavview = get_config('ipod_navigation', 'coursenavitems');
       ...
       // $showcategories = ($showallcourses && $this->show_categories());
        ...
        if ($showcategories &&(is_siteadmin($USER->id)) ) {
           ...
           // $categories = $this->load_all_categories();
...
if ($coursenavview =='catandcourses'){
   ...
   * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
   */
class block_ipod_navigation extends block_base {
    ...
    function get_content() {
        ...
        block_ipod_navigation::$navcount++;

        // Check if this block has been docked
        if ($this->docked === null) {
            $this->docked = get_user_preferences('nav_in_tab_panel_globalnav'.block_ipod_navigation::$navcount, 0);
        }

        // Check if there is a param to change the docked state
        if ($this->docked && optional_param('undock', null, PARAM_INT)==$this->instance->id) {
            unset_user_preference('nav_in_tab_panel_globalnav'.block_ipod_navigation::$navcount);
            $url = $this->page->url;
            $url->remove_params(array('undock'));
            redirect($url);
        } else if (!$this->docked && optional_param('dock', null, PARAM_INT)==$this->instance->id) {
            set_user_preferences(array('nav_in_tab_panel_globalnav'.block_ipod_navigation::$navcount=>1));
            $url = $this->page->url;
        ...
        $navigation = new ipod_navigation($this->page);
        ...
        $renderer = $this->page->get_renderer('block_ipod_navigation');
