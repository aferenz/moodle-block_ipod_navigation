 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_ipod_navigation_renderer extends plugin_renderer_base {
    /**
    ...
    public function navigation_tree(global_navigation $navigation, $expansionlimit, array $options = array()) {
        ...
        $content = "<script src='http://code.jquery.com/jquery-1.9.0.min.js'></script>
                    <script type='text/javascript' src='{$CFG->wwwroot}/blocks/ipod_navigation/js/fg.menu.js'>
                    </script>
        ...
        $content .= '<a tabindex="0" href="#nav_menu" class="fg-button fg-button-icon-right ui-widget ui-state-default ui-corner-all" id="hierarchybreadcrumb"><span class="ui-icon ui-icon-triangle-1-s"></span>Explore</a>';
        $content .= '<div id="nav_menu" class="hidden">';
        $content .= $this->navigation_node(array($navigation), array(), $expansionlimit, $options);
    ...
    protected function navigation_node($items, $attrs=array(), $expansionlimit=null, array $options = array(), $depth=1) {
        ...
        $courseformat = get_config('ipod_navigation', 'courseformat');
        $currentcourse = get_config('ipod_navigation', 'hidecourses');
        ...
        foreach ($items as $item) {
            ...
            if (is_string($item->action) || empty($item->action) || ($item->type === navigation_node::TYPE_CATEGORY && empty($options['linkcategories']))) {
                ...
                $content = html_writer::link($item->action, $content);
                //$content = html_writer::link('#', $content);
               ...
               if ($courseformat =='short'){
                    $content = html_writer::link($item->action, $content, $attributes);
                   //$content = html_writer::link('#', $content);
               }elseif ($courseformat =='long'){
                    $content = html_writer::link($item->action, $title);
                  // $content = html_writer::link('#', $title);
                }
            }
